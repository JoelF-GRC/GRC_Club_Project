...
## Stretch Goal: the immutable vault
...
The vault itself resides in `week-4/vault/`, a small standalone Terraform config, not in the `grc-gate.yml`, since the brief treats it as a same-day demonstration rather than permanent pipeline infrastructure. Object Lock has to be turned on when a bucket is created, there's no way to enable it later, which is why this is its own throwaway bucket instead of reusing anything from week 1.

The retention mode mattered more than it looks. COMPLIANCE mode makes an object undeletable by anyone, including the account root, until the retention window ends. That would have made "tear it down the same day" impossible to honor. GOVERNANCE mode was used instead: retention still blocks a normal delete, but a caller holding `s3:BypassGovernanceRetention` can remove the object early. That's what made the same-day teardown (deletion) achievable rather than a corner cut.

The demonstration ran in AWS, not simulated:
...
5. Before tearing anything down, the lock was actually tested, not just trusted. An unqualified `delete-object` on the versioned bucket only added a delete marker, it didn't touch the locked version underneath. Deleting that specific version without `--bypass-governance-retention` came back `AccessDenied because object protected by object lock`, captured in `week-4/evidence/vault-delete-blocked-output.txt`. That's the actual proof the lock was functional, not just that the API accepted the configuration.
...
Week 3 made non-compliant infrastructure impossible to merge and attached evidence to every gate run. Week 4 asks the next question: once that evidence leaves Actions, how does anyone confirm the file in front of them is the one the gate actually produced, and not a copy edited after the fact? Signing and the verify script turn "trust the evidence" into "check the evidence," the same shift Week 3 made for the underlying infrastructure.

The lesson repeated more literally than expected. Week 3's Conftest install checks the downloaded binary against a published checksum before trusting it; the first version of Week 4's own integrity check made the mistake that guards against, `sha256sum -c` matches by filename, which broke the moment the tamper test handed it a renamed copy, so it was rewritten to compare hashes directly instead of trusting a name. Same principle both times, verify by hash, not by name, just applied to a different artifact.
